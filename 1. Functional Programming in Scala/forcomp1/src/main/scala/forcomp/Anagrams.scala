...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy((c: Char) => c).map(r => (r._1, r._2.length)).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.foldLeft(x)(
      (xs: List[(Char, Int)], yi: (Char, Int)) => xs match{
        case List() => List(yi)
        case _ => xs.updated(xs.indexOf(yi._1), (yi._1, xs.filter( h => h._1 == yi._1).head._2 - yi._2))
      }
    )
